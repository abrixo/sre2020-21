...
def countfiles( lsttokens, repo, filenames, names, dates):
    ...
    ct = 0  # token counter
    try:
        print("Abraham here, passed test 1")
        # loop though all the commit pages until the last returned empty page
        while True:
            print("Abraham here, passed test while loop")
            if ct == len(lstTokens):
                ct = 0
            spage = str(ipage)
            ...
            ct += 1
            print("Abraham here, passed test 2")
            print("Abraham here, passed test 1")
            content = requests.get(commitsUrl)
            print("Abraham here, passed test 3")
            # jsonCommits = json.loads(content.content)
            # break out of the while loop if there are no more commits in the pages
            print("Abraham here, passed test 4")
            print("Abraham here, passed test 5")
            if len(jsonCommits) == 0:
                break
            # iterate through the list of commits in a page
            for shaObject in jsonCommits:
                print("Abraham here, passed test 6")
                sha = shaObject['sha']
                print("Abraham here, passed test 7")
                if ct == len(lstTokens):
                    print("Abraham here, passed test 8")
                    ct = 0
                # For each commit, use the GitHub commit API to extract the files touched by the commit
                print("Abraham here, passed test 9")
                shaUrl = 'https://api.github.com/repos/' + repo + '/commits/' + sha
                shaDetails, ct = github_auth(shaUrl, lsttokens, ct)
                
                print("Abraham here, passed test 10")
                ct += 1
        ...
        print("Abraham correct from here 'exception'")
